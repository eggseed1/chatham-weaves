...
export function PieceGallery({ images, name }: PieceGalleryProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {images.length > 1 && (
        <ul className="flex gap-3 overflow-x-auto pb-1" aria-label="Piece photographs">
          {images.map((img, i) => (
            <li key={img.src} className="shrink-0">
              <button
                ...
                className={`relative block h-20 w-16 overflow-hidden bg-linen transition-opacity ${
                  i === active ? "opacity-100 ring-1 ring-navy" : "opacity-60 hover:opacity-100"
                }`}
              >
                <Image
                  src={img.src}
